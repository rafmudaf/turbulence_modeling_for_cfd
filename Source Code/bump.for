...
c=======================================
      ...
      call prntff
c=======================================
c    Close bump.prt and input2c.dat
c=======================================
      close(iunit1)
      close(iunit2)
c=======================================
...
c=======================================================================
      ...
      do 30 j=1,jlp1
        ...
        do 20 i=1,ilp1
          x(i,j)=xx(i)
          ratio=(hmax+r0(1)-r0(i))/(hmax+.5*(yy(jlp1)-yy(jl)))
          if(j.eq.1) then
            y(i,j)=r0(i)-.5*(yy(3)+yy(2))*ratio
          else
            y(i,j)=r0(i)+.5*(yy(j)+yy(j-1))*ratio
          endif
 20     continue
 30   continue
      do 50 j=1,jl
...
c=======================================================================
      ...
      common/extra/r0(imxp1),xx(imxp1),yy(jmxp1)
      dimension rad0(99),xx0(99)
      data xx0/-.0624,-.0540,-.0461,-.0385,-.0313,-.0244,-.0178,-.0116,
     *         -.0057,.0000,.0057,.0118,.0186,.0259,.0339,.0426,.0521,
     *          .0625,.0738,.0861,.0995,.1142,.1301,.1475,.1665,.1855,
     *          .2028,.2186,.2330,.2461,.2581,.2690,.2790,.2881,.2964,
     *          .3040,.3109,.3172,.3230,.3282,.3330,.3388,.3445,.3503,
     *          .3560,.3618,.3675,.3733,.3790,.3848,.3906,.3963,.4021,
     *          .4078,.4136,.4193,.4251,.4308,.4366,.4424,.4481,.4539,
     *          .4596,.4654,.4711,.4769,.4827,.4884,.4942,.4999,.5057,
     *          .5114,.5172,.5229,.5287,.5345,.5402,.5460,.5517,.5575,
     *          .5632,.5690,.5747,.5805,.5863,.5920,.5978,.6035,.6093,
     *          .6150,.6208,.6265,.6323,.6381,.6438,.6496,.6553,.6611,
     *          .6668/
      data rad0/.2493,.2494,.2497,.2501,.2506,.2513,.2520,.2528,.2536,
     *    .2545,.2555,.2567,.2581,.2598,.2618,.2642,.2671,.2706,.2744,
     *    .2780,.2816,.2853,.2890,.2928,.2965,.2998,.3024,.3045,.3062,
     *    .3076,.3086,.3095,.3101,.3106,.3110,.3112,.3114,.3115,.3116,
     *    .3117,.3117,.3117,.3116,.3115,.3114,.3112,.3110,.3108,.3105,
     *    .3102,.3099,.3095,.3091,.3086,.3081,.3076,.3071,.3065,.3058,
     *    .3052,.3045,.3037,.3029,.3021,.3013,.3004,.2994,.2985,.2975,
     *    .2964,.2953,.2942,.2931,.2919,.2906,.2894,.2881,.2867,.2853,
     *    .2839,.2824,.2809,.2794,.2778,.2762,.2745,.2728,.2710,.2692,
     *    .2674,.2655,.2636,.2617,.2597,.2576,.2555,.2534,.2512,.2493/
      ibump=99
c=======================================================================
c          Horizontal Mesh
c=======================================
      hbump=xx0(ibump)-xx0(1)
      deta=1./(istep-iup+1)
      dx=hbump*deta
      ckgrid=1.
      do 10 i=iup,iup+istep
        xx(i)=(i-iup)*dx+xx0(1)
 10   continue
      hup=-xin
      dxup=dx
      deta=1./(iup-1)
      call scaler(deta,dxup,hup,ck)
      ckgrid=exp(ck*deta)
      ...
      dxsum=0.
      do 20 i=2,iup
        xx(iup-i+2)=xx0(1)-hup*(exp((i-2)*ck*deta)-1.)/ekm1
        dxcell=hup*(exp((i-1)*ck*deta)-1.)/ekm1-dxsum
        dxsum=dxsum+dxcell
 20   continue
      xx(1)=2.*xx(2)-xx(3)
      hdn=rl-hup-hbump-xx0(1)
      dxdn=dx
      deta=1./(il-iup-istep)
      call scaler(deta,dxdn,hdn,ck)
      ckgrid=exp(ck*deta)
      ...
      dxsum=0.
      xstart=xx(iup+istep)
      do 30 i=iup+istep+1,il
        xx(i)=hdn*(exp((i-iup-istep)*ck*deta)-1.)/ekm1+xstart
        dxcell=hdn*(exp((i-iup-istep+1)*ck*deta)-1.)/ekm1-dxsum
        dxsum=dxsum+dxcell
 30   continue
c=======================================
c Interpolate to determine local radius
c=======================================
      i0=1
      i1=2
      do 50 i=1,il
 40     ichange=0
        if(xx(i).le.xx0(1)) then
          r0(i)=rad0(1)
        elseif(xx(i).ge.xx0(ibump)) then
          r0(i)=rad0(ibump)
        elseif(xx(i).lt.xx0(i1)) then
          fract=(xx(i)-xx0(i0))/(xx0(i1)-xx0(i0))
          r0(i)=rad0(i0)+fract*(rad0(i1)-rad0(i0))
        else
          i0=i1
          i1=i0+1
          ichange=1
...
c=======================================
 ...
 60   continue
      ...
      jlfmp=jlfm+1
      hup=hmax-hfm
      dy=yy(jlfm)-yy(jlfm-1)
      deta=1./(jl-jlfmp)
      call scaler(deta,dy,hup,ck)
      ckgrid=exp(ck*deta)
      write(iunit1,130) ckgrid
      write(*,130) ckgrid
      ekm1=exp(ck)-1.
      dysum=0.
      do 70 j=jlfmp,jl
        yy(j)=yy(jlfm)+hup*(exp((j-jlfmp-.5)*ck*deta)-1.)/ekm1
        dycell=hup*(exp((j-jlfmp+1)*ck*deta)-1.)/ekm1-dysum
        dysum=dysum+dycell
 70   continue
      call prnt(iunit1)
c=======================================================================
 100  format(//,1x,'*****  The HORIZONTAL mesh has k = ',f8.4,
     *         ' upstream   of the bump')
 110  format(//,1x,'*****  The HORIZONTAL mesh has k = ',f8.4,
     *         ' downstream of the bump')
 120  format(//,1x,'*****  Fine  VERTICAL mesh has k = ',f8.4)
 130  format(//,1x,'*****  Upper VERTICAL mesh has k = ',f8.4)
c=======================================================================
      end
...
c----------------------------------
 ...
 10   continue
      iu(6)=10
      io=iu(1)
      open(io,file='bump.prt',form=pform,status=newfil)
      write(io,200)
      read(iin,100) a,iu(5)
      write(io,100) a,iu(5) 
c----------------------------------
...
c=======================================================================
 ...
 200  format(1x,'Program BUMP input data:'/,
...
c=======================================================================
      ...
      do 10 k=1,kmax
        write(iunit,110) k,xx(k),r0(k),k,yy(k)
 10   continue
      if(il.ne.jl) then
        kmaxp=kmax+1
        if(il.lt.jl) then
 ...
 20         write(iunit,120) k,yy(k)
          ...
          do 30 k=kmaxp,il
 30         write(iunit,130) k,xx(k),r0(k)
        endif
      endif
      return
c=======================================================================
 100  format(//,3x,'i',5x,'x(i)',5x,'r0(i)',20x,'j',5x,'y(j)'/)
 110  format(i4,1p2e12.4,14x,i4,e12.4)
 120  format(30x,i4,1pe12.4)
 130  format(i4,1p2e12.4)
c=======================================================================
      end
      subroutine prntff
c=======================================================================
